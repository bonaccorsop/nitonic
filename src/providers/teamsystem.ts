...
export class TeamSystem {
  ...
  public async uploadDocument(
    ...
    const validationResponse = await axios.post(
      'https://ts-console-api.agyo.io/xmlInvoices/extractBaseInfo',
      {
        invoices: [
          {
            id: `${fileName}0`,
        ...
        transmitterId: this.credentialsService.getCodiceFiscale(),
        flowType: 'SDI',
      },
      {
        headers: {
          ...
          pragma: 'no-cache',
          'x-app-name': 'ts-console.invoicesUpload',
          referrer: 'https://apps.agyo.io/',
        },
      },
    );

    // console.log(inspect(resp.data, true, null, true));
    const data = validationResponse.data[0];

    const senderId = data.senderId;
    const recipientId = data.recipientId;
    const transmissionFormat = data.transmissionFormat;
    ...
    const resp = await axios.post(`${this.baseUrl}/invoices`, payload, {
      headers: {
        ...this.resolveHeaders(),
        Accept: 'application/json, text/plain, */*',
        'x-app-name': 'ts-console.invoicesUpload',
  ...
  public async fetchContent(docId: string, format: 'xml' | 'pdf'): Promise<AxiosResponse> {
    return axios.get(`${this.baseUrl}/invoices/active/${docId}/content`, {
      params: format === 'xml' ? { format } : { format, type: 'asso' }, // type: ''asso' for different pdf layout
      headers: {
        ...this.resolveHeaders(),
      },
      responseType: format === 'xml' ? 'json' : 'arraybuffer',
    });
  }
